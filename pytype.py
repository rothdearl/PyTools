...
def print_lines(lines: list[str], *, has_newlines: bool) -> None:
    ...
    """
    line_number = 0
    line_start = len(lines) + LineInfo.line_start + 1 if LineInfo.line_start < 0 else LineInfo.line_start
...
def set_line_info_values() -> None:
    ...
    :return: None
    """
    LineInfo.line_start = 1 if not Program.args.line_start else Program.args.line_start[0]  # --line-start
    LineInfo.lines = sys.maxsize if not Program.args.lines else Program.args.lines[0]  # --lines
